...
class ShipCleanupProps(bpy.types.PropertyGroup):
    ...
    loose_planar_max_fraction: bpy.props.FloatProperty(
        name="Max flat-island size (fraction of object)", default=0.5,
        min=0.0, max=1.0,
...
def scope_objects(context):
    ...
    return [o for o in objs if o.name.startswith(REVIEW_PREFIX) is False]
            ...
            for i, isl in enumerate(islands):
                ...
                if not remove and p.loose_remove_planar_islands:
                    if island_is_planar(faces):
                        ...
                        if obj_diag <= 0 or (d / obj_diag) <= p.loose_planar_max_fraction:
                            remove = True
                if remove:
...
class SHIPCLEAN_OT_make_manifold(bpy.types.Operator):
    ...
    def voxel_remesh(self, context, obj, p):
        # Thicken open shells first so zero-thickness panels survive the
        # signed-distance conversion (they'd otherwise vanish or leak).
