...
DEF_DECAL_MAT_REGEX = r"(decal|_pom_|pom_decal|livery|_lines_|rtt_|_hud)"
DEF_GEAR_REGEX = r"^BONE_.*(foot|oleo|piston|strut|hinge|gear|wheel|skid)"
DEF_INTERNAL_REGEX = (
    r"(cockpit|seat(?!access)|joystick|pedal|dashboard|button|screen|hud"
    r"|knob|ejection|^control_|^rtt_)"
)
    ...
    manifold_method: bpy.props.EnumProperty(
        ...
        items=[
            ("ALPHAWRAP", "Alpha Wrap (CGAL via pymeshlab)",
             "State of the art: guaranteed watertight 2-manifold enclosing "
             "surface (Portaneri et al., SIGGRAPH 2022). Needs pymeshlab."),
            ("VOXEL", "Voxel Remesh (native OpenVDB)",
             "Volumetric remesh; robust, always available. Thin open shells "
             "get a Solidify pass first so they survive."),
        ],
        default="VOXEL",
    )
...
class SHIPCLEAN_OT_classify(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for o in meshes:
            ...
            if mats and all(iproxy_mat.search(m) for m in mats):
                proxies.append(o)
                taken.add(o)
            elif p.strip_internal_faces and any(iproxy_mat.search(m) for m in mats):
        ...
        self.report({"INFO"},
                    ...
                    f"{n_int} internals, {n_dup} parked duplicates, "
                    f"{n_prox} interior proxies "
...
class SHIPCLEAN_OT_make_manifold(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if p.manifold_method == "ALPHAWRAP":
            ...
            if not ok:
                ...
                self.voxel_remesh(context, joined, p)
        else:
            self.voxel_remesh(context, joined, p)

        me = joined.data
        state = "watertight" if not mesh_has_open_edges(me) else \
            "STILL HAS OPEN EDGES — reduce voxel size / raise solidify"
        self.report({"INFO"},
                    f"'{joined.name}': {len(me.polygons)} faces, {state}")
        return {"FINISHED"}

    # -- native volumetric path ------------------------------------------
    def voxel_remesh(self, context, obj, p):
        # 1) Pre-clean: merge micro-doubles and dissolve degenerate faces.
        #    Zero-area faces have garbage normals — they were one source of
        #    the needle spikes.
        bm = bmesh.new()
        ...
        # 2) Thicken open shells with the NON-MANIFOLD ("Complex") solidify
        #    in FIXED thickness mode. Unlike Simple+Even Offset — which
        #    divides the offset by the corner angle and shoots metre-long
        #    needles out of degenerate corners — Fixed mode moves every
        #    vertex by at most the thickness. Displacement is bounded, so
        #    spikes are impossible.
        if p.solidify_thickness > 0 and mesh_has_open_edges(obj.data):
        ...
        # 3) Volumetric remesh (OpenVDB signed distance field).
        obj.data.remesh_voxel_size = p.voxel_size
    ...
    def strip_enclosed_islands(self, context, obj):
        ...
        if len(survivors) > 1:
            bpy.ops.object.join()
        main.name = obj.name if main is not obj else main.name
