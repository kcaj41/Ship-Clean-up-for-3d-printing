...
class SHIPCLEAN_OT_make_manifold(bpy.types.Operator):
    ...
    # -- per-object sealing (memory-friendly) ----------------------------
    def build_sealed_shells(self, context, sources, p, wall):
        """Seal every source object individually, split islands into
        open/closed sets, thicken the open set inward, join to one object.
        Working per-object keeps peak memory low — pre-cleaning one giant
        joined mesh was the long-run-then-crash failure mode."""
        deps = context.evaluated_depsgraph_get()
        bm_open = bmesh.new()
        bm_closed = bmesh.new()
        ...
        for src in sources:
            ...
            # island partition: route closed islands and open islands
                ...
                while stack:
                    ...
                    for e in v.link_edges:
                        ...
                        if w.index not in seen:
                            ...
                            isl.append(w)
                            stack.append(w)
                (open_verts if is_open else closed_verts).extend(isl)

            for verts, acc in ((open_verts, bm_open),
                               (closed_verts, bm_closed)):
                if not verts:
                    continue
                bpart = bm.copy()
                ...
                tmp = bpy.data.meshes.new("tmp_route")
                bpart.to_mesh(tmp)
                bpart.free()
                acc.from_mesh(tmp)
        ...
        objs = []
        for name, acc in (("ShipPrint_open", bm_open),
                          ("ShipPrint_closed", bm_closed)):
        ...
        open_obj = next((o for o in objs
                         if o.name.startswith("ShipPrint_open")), None)
        if open_obj is not None and wall > 0:
            for o in context.selected_objects:
                o.select_set(False)
            open_obj.select_set(True)
            context.view_layer.objects.active = open_obj
            mod = open_obj.modifiers.new("SealSolidify", "SOLIDIFY")
