...
class SHIPCLEAN_OT_classify(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        def is_external_exempt(o):
            """Exterior hardware that fails ray-visibility tests: engine
            glow meshes recessed in nozzle ducts, gimbal turret parts
            half-inside their mounts (both misclassified as internal on
            the Polaris). Exempt from GEOMETRIC rules only — name and
            collection rules still apply."""
            if ext_re and ext_re.search(o.name):
                return True
            if ext_mat_re:
                mats = object_used_material_names(o)
                if mats and all(ext_mat_re.search(m) for m in mats):
                    return True
            return False
