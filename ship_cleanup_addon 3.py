...
class ShipCleanupProps(bpy.types.PropertyGroup):
    ...
    manifold_method: bpy.props.EnumProperty(
        name="Method",
        items=[
            ("BOOLEAN", "Exact Boolean Union (detail-preserving)",
             "Seal each shell, then exact CSG self-union (Blender's Exact "
             "solver). Keeps sharp detail and low poly counts; the result "
             "is only manifold if the sealing succeeds — check the report"),
            ...
            ("VOXEL", "Voxel Remesh (native OpenVDB)",
             ...
             "detail. Good fallback when Boolean fails"),
        ],
        default="BOOLEAN",
    )
...
class SHIPCLEAN_OT_make_manifold(bpy.types.Operator):
    ...
    def preclean(self, obj):
        ...
        # faces have garbage normals — they were one source of the spikes.
        bm = bmesh.new()
    ...
    def boolean_union(self, context, obj, p, wall):
        try:
            # 1) try to close simple holes so shells become volumes
            bm = bmesh.new()
            bm.from_mesh(obj.data)
            ...
            # the Manifold library — far faster than Exact, guaranteed
            # manifold output, but wants manifold inputs, which our sealed
            # shells now are). Try it first, fall back to Exact.
            try:
